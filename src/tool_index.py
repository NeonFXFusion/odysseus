...
ASSISTANT_ALWAYS_AVAILABLE = frozenset({
    "list_email_accounts", "list_emails", "search_emails", "read_email", "extract_email_urls", "send_email", "reply_to_email",
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "ui_control": "Control the UI and toggle tools on/off. Use this to turn off / turn on / disable / enable individual tools and features: shell (bash), search (web), research, browser, documents, incognito. Open panels (documents library, gallery, email inbox, instagram DMs, sessions, notes, memories/brain, skills, settings, cookbook) via `open_panel <name>`. Use `open_email_reply <uid> <folder> reply` to open an email reply draft document without sending. Also switches between chat/agent modes, changes the current model, and applies/creates themes.",
    ...
    "search_emails": "Search emails by sender, subject, and body text across INBOX, Sent, and Archive/All Mail by default. Use instead of list_emails when the user asks to find mail from a person/company/sender, about a topic, matching an invoice/order/property/thread/invitation, or containing specific text. Use for requests like find Amazon Prime email or find eBay message; do not resolve those senders as contacts. Returns UID, folder, and account so read_email or reply_to_email can follow. For URL/link/tracking/package/delivery queries, results also include extracted_urls, url_details, and tracking_candidates when available.",
    "read_email": "Read the full content of a specific email by UID or Message-ID. View email body, attachments, and extracted URL metadata (extracted_urls, url_details, tracking_candidates). Supports account from list_email_accounts when the UID belongs to a non-default mailbox.",
    "extract_email_urls": "Extract HTTP/HTTPS URLs from an email body or a mailbox email by UID/Message-ID. Labels each URL with alt text, button text, anchor text, or standalone url. Use directly after search_emails with UID/folder/account when the user asks for tracking/open/verify links, URLs, buttons, or link context inside an email; read_email is not required first.",
    ...
    "list_instagram_accounts": "List configured Instagram private API accounts and default status. Credentials/session IDs are never returned. Use before Instagram DM work when the user names a specific IG account.",
    "list_instagram_threads": "List recent Instagram DM threads via the private API. Shows participants, thread_id, latest previews, and extracted URL metadata when present.",
    "search_instagram_messages": "Search Instagram DMs by participant, thread title, message text, or item type. Use instead of listing many threads and filtering manually. Returns thread_id, message_id, account, text, extracted_urls, url_details, and tracking_candidates.",
